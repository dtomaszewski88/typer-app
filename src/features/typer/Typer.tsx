...
const Typer = ({ score,
   errors,errorTime, countDown, localText, 
   remoteText, currentWord, remainingWords, updateLocalText, countDownInit }: Props) => {
  ...
  (
    <div className='word-wrapper-inner'>
      <div className="prompt-wrapper">
          {map(promptLetters, (letter: string, index: number) => <div className={classNames('letter', {'next': index === textLetters.length})} key={`${letter}-${index}`}>{letter}</div>)}
      </div>
      <div className="text-wrapper">
        {map(textLetters, (letter: string, index: number) => <div className='letter' key={`${letter}-${index}`}>{letter}</div>)}
      </div>
    </div>
  )
    ...
    <Container>
      ...
      <div className='typer'>
          <div className='word-wrapper' key={errorTime}>
            <CSSTransition in={Boolean(errorTime)} classNames='error-input' timeout={500} appear>
              {wordWrapper}
            </CSSTransition>
          </div>
          <div className='remaining-words'>
            {map(remainingWords, (word: string, index: number) => {
              ...
              return <div key={word} className={classNames('remaining-word', `remaining-word-${index}`)} style={{
