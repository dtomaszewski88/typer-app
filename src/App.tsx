...
export default function App(props: Props) {
  const { updateText, countDownInit, status, addItems, removeItem, listKey, resetKey, resetAnimate, toggleAnimate } = props;
  const handleAddMultiple = () => {
    setTimeout(() => {addItems(1, false)}, 0)
    setTimeout(() => {addItems(3, false)}, 0)
    setTimeout(() => {addItems(5, false)}, 500)
  }
  return (
    <>
      <KeyboardEventHandler handleKeys={['enter']} onKeyEvent={() => countDownInit(3)} />
      <KeyboardEventHandler handleKeys={['alphabetic']} onKeyEvent={(key, event) => updateText(key)} />
